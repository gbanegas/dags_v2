...
/*
 * ALTERNANT DECODING: Take as input v, y compute the syndrome whithout the the matrix the received word code_word.
 */
               ...
               unsigned char *code_word)
{
    ...
    for (i = 0; i < code_length; i++)
    {
        if ((v[i] != 0) && (poly_eval(sigma, gf_inv(v[i])) == 0))
        {
    ...
    for (j = 0; j <= pos->deg; j++)
    {
        ...
        for (i = 0; i <= pos->deg; i++)
        {
            if (i != j)
            {
                tmp = gf_mult(v[pos->coeff[i]],
                              gf_inv(v[pos->coeff[j]]));
                tmp = gf_add(1, tmp);
                pol_gf = gf_mult(pol_gf, tmp);
            }
        }
        o = poly_eval(omega, gf_inv(v[pos->coeff[j]]));
        tmp1 = gf_mult(y[pos->coeff[j]], pol_gf);
        app->coeff[j] = gf_mult(o, gf_inv(tmp1));
    }
    poly_calcule_deg(app);
    //free(v);
